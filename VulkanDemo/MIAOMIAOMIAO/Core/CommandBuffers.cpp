...
namespace Rendering
{
	CommandBuffers::CommandBuffers(const CommandPool& commandPool, uint32_t size)
		: device(commandPool.GetDevice()), commandPool(commandPool)
	{
		...
		VK_CHECK(vkAllocateCommandBuffers(device.Get(), &allocInfo, commandBuffers.data()), "Allocate command buffers");
	}
	...
	CommandBuffers::~CommandBuffers()
	{
		...
		{
			vkFreeCommandBuffers(device.Get(), commandPool.Get(), static_cast<uint32_t>(commandBuffers.size()), commandBuffers.data());
