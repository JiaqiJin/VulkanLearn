...
private:
	...
	VRaii<VkPipeline> depth_pipeline;
...
{
	vkDestroyPipeline(device, depth_pipeline.get(), nullptr);
	vkDeviceWaitIdle(graphics_device);
}
...
void _VulkanRenderer_Impl::createGraphicsPipelines()
{
	...
	// create main pipeline
	{
...

		{
			...
			if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &depth_pipeline.get()) != VK_SUCCESS) 
			{
				throw std::runtime_error("failed to create graphics pipeline!");
			}
		}
