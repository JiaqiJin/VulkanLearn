...
namespace RHI
{
	...
	public:
		...
		std::shared_ptr<MemoryKey> AllocateBufferMemChunk(const Buffer* pBuffer, uint32_t memoryPropertyBits, const void* pData = nullptr);
		std::shared_ptr<MemoryKey> AllocateImageMemChunk(const Image* image, uint32_t memoryPropertyBits, const void* pData = nullptr);
